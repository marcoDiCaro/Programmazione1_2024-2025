=========================================================================
ISTRUZIONI per il PRIMO ASSEGNAMENTO PYTHON aa 2024/25
========================================================================
L'assegnamento prevede la realizzazione di un insieme di funzioni per la gestione di 
un insieme di animali di uno zoo come specificato nel file zoo.pdf.

Come procedere:
(1) realizzare (nel file zoo.py) le funzioni richieste
...
(2) eseguire i test nel file main.py fornito dai docenti (senza modificarlo)
    per effettuare questo test sia main.py che testMy.py che zoo.py
    si devono trovare nella stessa cartella
(3) se tutti i test vengono superati (Pass) e viene stampato il messaggio finale 
    che invita alla consegna, preparare il file zoo.py per
    la consegna inserendo nome cognome e contatti dei componenti del gruppo
    nell'intestazione.
(4) consegnare inviando il file Python (o txt) zoo.py (ed eventuali file
    aggiuntivi -- vedi istruzioni sotto) sul Moodle del corso alla consegna
    chiamata "Assegnamento 2 Python Base"

------------------------------------------------
Cosa contiene la cartella dell'assegnamento
------------------------------------------------

zoo.pdf		: file contenente la descrizione dell'assegnamento

zoo.py 		: file che contiene la definizione delle funzioni da realizzare
...
di ispirarsi a main.py per creare dei test propri che permettano di
...
da consegnare insieme a zoo.py.
...
La modalità di valutazione degli assegnamenti è descritta in zoo.pdf.
